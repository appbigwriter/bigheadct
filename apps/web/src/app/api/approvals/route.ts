...
export async function GET() {
  ...
  try {
    return NextResponse.json(await authenticatedApi<unknown>("/v1/approvals?queue=all", { organizationId }), { headers: { "cache-control": "no-store" } });
  } catch (error) {
    const status = error instanceof BigHeadApiError ? error.status : 500;
    return NextResponse.json({ detail: error instanceof Error ? error.message : "Falha ao carregar aprovacoes." }, { status });
  }
}
