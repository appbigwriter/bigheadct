...
export async function GET(_request: Request, context: { params: Promise<{ approvalId: string }> }) {
  ...
  try {
    return NextResponse.json(await authenticatedApi<unknown>(`/v1/approvals/${encodeURIComponent(approvalId)}`, { organizationId }), { headers: { "cache-control": "no-store" } });
  } catch (error) {
    const status = error instanceof BigHeadApiError ? error.status : 500;
    return NextResponse.json({ detail: error instanceof Error ? error.message : "Falha ao carregar aprovacao." }, { status });
  }
}
