-- BigHead V3 - schema Supabase/PostgreSQL
-- Status: baseline para revisao. Aplicar primeiro em projeto local descartavel.
-- Premissas: banco vazio; PostgreSQL 15+; Supabase Auth/Storage; embedding 1536.
-- Antes de producao: validar dimensao do embedding, LGPD, matriz de papeis,
-- executar pgTAP, supabase db advisors e teste integral de RLS cross-tenant.
...
create type public.task_status as enum ('new','triaged','in_progress','waiting_tool','waiting_human','ready_for_review','approved','failed','done','canceled');
create type public.run_status as enum ('queued','running','waiting','succeeded','failed','canceled','dead_letter');
create type public.approval_status as enum ('pending','approved','rejected','changes_requested','expired','canceled');
create type public.memory_kind as enum ('fact','inference','decision','summary');
create type public.review_status as enum ('draft','pending','approved','contested','expired','archived');
create type public.content_status as enum ('draft','review','approved','scheduled','published','failed','archived');
create type public.lead_status as enum ('new','qualified','nurturing','opportunity','won','lost','disqualified');
create type public.experiment_status as enum ('draft','running','paused','completed','canceled');
...
begin
  foreach t in array array[
    'profiles','organizations','organization_members','rooms','model_providers','agents','skills',
...
-- -----------------------------------------------------------------------------
-- RLS. Leitura para membro; escrita para papeis operacionais; administracao
-- sensivel restrita. O backend service_role continua sujeito a regras da API.
-- -----------------------------------------------------------------------------
...
create policy organizations_select on public.organizations for select to authenticated using (private.current_user_is_member(id));
create policy organizations_update on public.organizations for update to authenticated using (private.current_user_has_role(id, array['owner','admin']::public.member_role[])) with check (private.current_user_has_role(id, array['owner','admin']::public.member_role[]));

alter table public.organization_members enable row level security;
create policy organization_members_select on public.organization_members for select to authenticated using (private.current_user_is_member(organization_id));
-- Membership e papel sao alterados somente pelo backend, que deve impedir a
-- remocao/rebaixamento do ultimo owner e registrar auditoria.
...
begin
  foreach t in array array[
    'organization_invites','model_providers','models','agents','agent_versions',
    ...
    'campaigns','content_assets','experiments','experiment_variants','analytics_events','notifications',
...
using (author_user_id = (select auth.uid()) and deleted_at is null)
...
declare t text;
begin
  foreach t in array array['tasks','task_dependencies','artifacts','approval_requests','approval_decisions','knowledge_documents','memory_items','crm_accounts','crm_contacts','leads','lead_signals','opportunities','campaigns','content_assets','experiments','experiment_variants'] loop
...
grant select, insert, update, delete on table
  ...
  public.workflows, public.workflow_versions, public.playbooks, public.tasks,
  public.task_dependencies, public.artifacts, public.approval_requests, public.approval_decisions,
  ...
  public.content_assets, public.experiments, public.experiment_variants, public.notifications,
...
-- -----------------------------------------------------------------------------
-- Storage privado. Caminho obrigatorio: <organization_id>/<uuid>/<arquivo>.
-- Bucket criado idempotentemente; policies usam o primeiro segmento como tenant.
-- -----------------------------------------------------------------------------

insert into storage.buckets(id, name, public, file_size_limit)
values ('bighead-private', 'bighead-private', false, 52428800)
on conflict (id) do update set public = false, file_size_limit = excluded.file_size_limit;

create policy bighead_storage_select on storage.objects for select to authenticated using (
  bucket_id = 'bighead-private'
  and private.current_user_is_member(private.try_uuid((storage.foldername(name))[1]))
);
create policy bighead_storage_insert on storage.objects for insert to authenticated with check (
  bucket_id = 'bighead-private'
  and private.current_user_is_member(private.try_uuid((storage.foldername(name))[1]))
);
create policy bighead_storage_update on storage.objects for update to authenticated using (
  bucket_id = 'bighead-private'
  and private.current_user_is_member(private.try_uuid((storage.foldername(name))[1]))
) with check (
  bucket_id = 'bighead-private'
...
create policy bighead_storage_delete on storage.objects for delete to authenticated using (
  bucket_id = 'bighead-private'
  and private.current_user_has_role(private.try_uuid((storage.foldername(name))[1]), array['owner','admin','manager']::public.member_role[])
);

commit;

-- REVISAO OBRIGATORIA ANTES DE APLICAR
-- 1. Confirmar extensions.vector(1536) com o modelo de embedding escolhido.
-- 2. Implementar trigger seguro de bootstrap profile/owner em migration separada.
-- 3. Implementar protecao do ultimo owner e deteccao de ciclo em task_dependencies.
-- 4. Decidir se o frontend acessara Data API diretamente; se nao, reduzir grants.
-- 5. Testar nomes/policies existentes em storage.objects antes de reaplicar.
-- 6. Rodar pgTAP para RLS, state machine, leases, aprovacao e isolamento por tenant.
